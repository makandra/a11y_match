...
async function startAudit(auditOptions) {
  ...
  const { tags, includedRules } = auditOptions["included_rules"].reduce(
    (intermediateValue, currentRule) => {
      if (tagRuleKeywords[currentRule])
        intermediateValue.tags = [
          ...intermediateValue.tags,
          ...tagRuleKeywords[currentRule],
        ]
      else intermediateValue.includedRules.push(currentRule)
  ...
  const rules = {
    ...(runOnly["type"] === "rule"
      ? []
      : Object.fromEntries(
          includedRules.map((rule) => [rule, { enabled: true }])
        )),
    ...Object.fromEntries(
      auditOptions["excluded_rules"].map((rule) => [rule, { enabled: false }])
    ),
  }
    ...
    (intermediateResult, assertion) => {
      ...
      if (
        !["earl:failed", "earl:cantTell"].some(
          (passingOutcome) => passingOutcome === outcome
        )
      )
        return intermediateResult
      const key =
        assertion["test"]["@id"] + assertion["result"]["info"] + outcome
      if (key in intermediateResult[outcome]) {
        ...
        intermediateResult[outcome][key] = {
          ...
          occurences: [assertion["result"]["pointer"]],
        }
      }
      return intermediateResult
    },
