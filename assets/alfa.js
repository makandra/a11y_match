...
const startAudit = async () => {
    ...
    const {wcagLevel, otherRequirementTypes, includedRules, miscRules} =
        auditOptions["included_rules"].reduce((intermediateValue, currentRule) => {
            const wcagConformance = wcagRuleKeywords[currentRule]
            if (wcagConformance)
                intermediateValue.wcagLevel = wcagConformance
                ...
                if (otherRequirementType)
                    ...
                    includedRules.push(currentRule)
            }
            return intermediateValue
        }, {wcagLevel: undefined, miscRules: false, otherRequirementTypes: [], includedRules: []})
    ...
    const filteredRules = rules.filter(
        rule => {
            ...
            if (includedRules.includes(ruleId))
                return true
            ...
            else if (isNotCriterion(rule) && rule.hasRequirement(or(...otherRequirementTypes)))
                return true

            // Check if rule is without any requirements
            else if (miscRules && rule.requirements.length === 0)
                return true
        }
    )
    ...
    const outcomes = [...await Audit.of(input, filteredRules, oracle).evaluate()]
